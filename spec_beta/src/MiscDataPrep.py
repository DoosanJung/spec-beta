...
class MiscDataPrep(object):
    ...
    def __init__(self, start_mo, end_mo):
        ...
        self.reg_mo_lst = rdp.get_reg_mo_lst()
    ...
    def get_m_stock_level_disp(self):
        ...
        try:
            ...
            self.m_stock_level_disp = m_stock_level_disp[m_stock_level_disp.index < self.reg_mo_lst.iloc[-1]+relativedelta(months=1)]
    ...
    def get_monthly_mkt_cap(self):
        ...
        try:
            ...
            mkt_cap = mkt_cap[(mkt_cap.index >= self.reg_mo_lst[0]) & (mkt_cap.index < self.reg_mo_lst.iloc[-1]+relativedelta(months=1))]

            # mkt cap at last day of the months
            self.m_mkt_cap = mkt_cap.groupby([lambda x: x.year,lambda x: x.month]).last()
            self.m_mkt_cap.index = self.reg_mo_lst.apply(lambda x : x + relativedelta(months=1) - timedelta(days=1))
            if self.to_csv == True:
    ...
    def get_monthly_vol(self):
        ...
        try:
            ...
            vol = vol[(vol.index >= self.reg_mo_lst[0]) & (vol.index < self.reg_mo_lst.iloc[-1]+relativedelta(months=1))]

            # vol at last day of the months
            self.m_vol = vol.groupby([lambda x: x.year,lambda x: x.month]).last()
            self.m_vol.index = self.reg_mo_lst.apply(lambda x : x + relativedelta(months=1) - timedelta(days=1))
            if self.to_csv == True:
    ...
    def get_smb_hml_umd(self):
        ...
        try:
            ...
            SMB_wo_wknds = SMB_wo_wknds[(SMB_wo_wknds.index >= self.reg_mo_lst[0]) & (SMB_wo_wknds.index < self.reg_mo_lst.iloc[-1]+relativedelta(months=1))]
            HML_wo_wknds = HML_wo_wknds[(HML_wo_wknds.index >= self.reg_mo_lst[0]) & (HML_wo_wknds.index < self.reg_mo_lst.iloc[-1]+relativedelta(months=1))]
            ...
            UMD_wo_wknds = UMD_wo_wknds[(UMD_wo_wknds.index >= self.reg_mo_lst[0]) & (UMD_wo_wknds.index < self.reg_mo_lst.iloc[-1]+relativedelta(months=1))]
    ...
    def get_D_P(self):
        ...
        try:
            ...
            monthly_DP_KOSPI = monthly_DP_KOSPI[(monthly_DP_KOSPI.index >= self.reg_mo_lst[0]) & (monthly_DP_KOSPI.index < self.reg_mo_lst.iloc[-1]+relativedelta(months=1))]
    ...
    def get_inflation(self):
        ...
        try:
            ...
            self.monthly_CPI_yearly_inflation = monthly_CPI_yearly_inflation[(monthly_CPI_yearly_inflation.index >= self.reg_mo_lst[0]) \
                                            & (monthly_CPI_yearly_inflation.index < self.reg_mo_lst.iloc[-1]+relativedelta(months=1))]
            if self.decimal_unit == True:
    ...
    def get_CP_CD_spread(self):
        ...
        try:
            ...
            CP_CD_spread = CP_CD_spread[(CP_CD_spread.index >= self.reg_mo_lst[0]) & (CP_CD_spread.index < self.reg_mo_lst.iloc[-1]+relativedelta(months=1))]

            # last day of the month
            m_CP_CD_spread = CP_CD_spread.groupby([lambda x: x.year,lambda x: x.month]).last()
            m_CP_CD_spread.index = self.reg_mo_lst.apply(lambda x : x + relativedelta(months=1) - timedelta(days=1))
